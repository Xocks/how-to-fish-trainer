...
- **当前代码版本：** `0.3.0rc1`（真实游戏验证后再创建 RC 标签）
...
| **F9 枪械锁头** | 持枪 ADS 并按住右键时锁定生物头部；好友/私有测试开关打开后也可选择其他玩家。默认 120 米、12°获取角度并带平滑和遮挡检测。 |
| **F11 物品/生物标签** | 在游戏画面显示名称、距离和分类颜色；遮挡目标变暗，默认最多 200 个、150 米。 |
| **Insert 鼠标面板** | 打开战斗、透视、实验和诊断标签页，可用鼠标调整距离、角度和平滑度。 |
...
| **F9** | **枪械锁头** | 持枪 ADS + 按住右键时锁定头部；松开、换枪、死亡或打开菜单立即释放。 |
| **F11** | **物品 / 生物透视** | 开关名称、距离、分类颜色和遮挡变暗标签。 |
| **Insert** | **鼠标控制面板** | 显示或隐藏鼠标面板；关闭面板时恢复游戏鼠标锁定状态。 |
...
2. **JIT 编译与内存动态缩放 (`howtofish_cheat.mono.patcher`)**：
   ...
   - **物品生成器 (`F7` / `F8`)**：精确解析 `GameInfo.GetSpawnable(byte)`，先执行 `SpawnSafety` 分类；房主生成仍使用一次性主线程握手。普通客户端只允许私有房安全 RPC 探针，绝不伪造 `IsServerInitialized`。
   - **锁头 / ESP (`F9` / `F11`)**：外部控制器将仓库内辅助程序集临时加载进当前 Mono 域，由 Unity 主线程扫描 `CreatureManager`、`PlayerManager` 与 `ItemManager`。辅助程序集不复制到游戏目录，退出时销毁运行时对象并恢复补丁。
