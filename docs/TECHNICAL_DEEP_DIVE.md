...
4. F8 verifies `NetworkBehaviour.IsServerInitialized`, creates and pins a managed Mono string, then installs a one-shot gate on `Player.LateUpdate` so `DazedCommands.UseSpawnCommand(name, false)` runs on Unity's main thread. After the call, the main thread signals completion and spins inside scratch memory; the trainer restores and flushes the original `LateUpdate` bytes, then signals the main thread to return. This handshake prevents either thread from executing a partially restored prologue.
