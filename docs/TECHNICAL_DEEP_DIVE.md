...
The Python controller remains an external process. It validates the exact game assembly before patching, loads the release-unique `HowToFishTrainer.Runtime.V030.dll` into the existing Mono domain from the trainer directory, and dispatches Unity initialization from a one-shot `Player.LateUpdate` gate. The unique assembly identity prevents Mono from silently reusing an older helper that cannot be unloaded until the game exits. No DLL is copied into the game installation. Aim target enumeration, `Camera.WorldToScreenPoint`, physics obstruction checks, preview construction, third-person camera placement, avatar-template discovery, native avatar presentation, and projectile snapshot updates run only on Unity's main thread; remote worker threads only update primitive control flags or read immutable catalog strings. Live `post2` evidence showed the local body head could be about 400 metres from `CurCam`, so the active camera remains the only trusted world-space pivot. Live `post4` evidence then proved that owner initialization destroys the remote character presentation; `post5` therefore began rebuilding only its render data. Post7 diagnostics exposed two concrete defects: a cumulative root-position addition caused indefinite vertical drift, and reflection invoked the game's `ResolveIK(bool)` method with zero arguments. The v0.3.0 presentation backend removes the invented gait entirely, reconstructs only the destroyed presentation references, temporarily rebinds the owner's retained `PlayerBody`, `PlayerHands`, `PlayerLegs`, `OtherPlayer` and four IK solvers, and runs the game's own remote-avatar order using real local movement, held-item, camera, breathing and ground state. All rebound fields and component enabled states are restored before the display copy is destroyed. The local presentation remains explicitly distinct from remote-client and hitbox proof.

The helper provides world-distance 360-degree nearest-target aim with stable-first and rate-limited continuous motion filters, 60 Hz ESP projection, a dual-backend collision-shortened third-person camera, an Insert panel, and experimental silent-ballistics paths. Fish and birds are enabled by default; `BirdManager._flyingBirds` is merged with alive creatures by instance ID, while other creatures and consent-gated players are optional. Third person substitutes the active camera pose only inside SRP or built-in render callbacks and restores it immediately afterward. A render-only avatar mirror copies the selected network character's transform hierarchy, shared meshes/materials, blend shapes, and skinned bone mapping; it copies no `MonoBehaviour`, network component, collider, rigidbody, or animator. Source bones are synchronized before rendering and the mirror is destroyed on every teardown path. If no render callback is observed for 30 focused frames, the camera still falls back to `PlayerDeathCam._deathCam` through `Player.SetCurCam` without invoking death-only methods. The two private-room pose modes are inserted before the existing pre-tick send subscribers and restore camera rotation on post-tick. A pre-update component applies a target direction before the original `Weapon.Update`/`Shoot` path and the post-update component restores the fire point. Entity projectiles are filtered to `Owner == Player.LocalPlayer && IsLocal`, retain velocity magnitude, bind one target per shot, and stop steering at obstruction. Server projectile acceptance, remote pose/hitbox behavior, and live avatar presentation remain game-test questions, not static-analysis claims. Shutdown restores camera, input, renderer, pose, fire-point state, and render callbacks.
...
2. `Item.GetName()`, `UnityEngine.Object.get_name()`, and `Item.get_IsQuestItem()` provide display, lookup, and risk metadata. Build 24911270 leaves the prefab-only, non-serialized `Item._type` at zero, so catalog classification falls back to the verified creature ID layout and weapon spawn keys.
3. F7 displays the catalog and stores a validated selection. Quest and unknown items require a second confirmation.
4. F8 verifies `NetworkBehaviour.IsServerInitialized`, creates and pins one managed Mono string per spawn key, then installs a one-shot gate on `Player.LateUpdate` so `DazedCommands.UseSpawnCommand(name, false)` runs on Unity's main thread. Build 24911270's native crash report maps directly to `mono_gchandle_free`, so these strings remain process-lifetime roots and are reused; retention is bounded by the 85-key runtime catalog. After the command, the main thread signals completion and spins inside scratch memory while the trainer restores and flushes the original `LateUpdate` bytes. The trainer finally releases the main thread to return, preventing either thread from executing a partially restored prologue.
