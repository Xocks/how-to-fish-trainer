...
4. F8 verifies `NetworkBehaviour.IsServerInitialized`, creates and pins a managed Mono string, then installs a one-shot gate on `Player.LateUpdate` so `DazedCommands.UseSpawnCommand(name, false)` runs on Unity's main thread. The same already Mono-attached thread releases the GC handle immediately after the command returns; no post-spawn remote Mono thread is created. It then signals completion and spins inside scratch memory while the trainer restores and flushes the original `LateUpdate` bytes. The trainer finally releases the main thread to return, preventing either thread from executing a partially restored prologue.
